#!/usr/bin/env sh
# install.sh — install the d (Deno version manager) binary
set -e
...
printf "  ${CYAN}export PATH=\"\$HOME/.local/bin:\$PATH\"${NC}   # for the d binary\n"
printf "  ${CYAN}export PATH=\"\$HOME/.d/bin:\$PATH\"${NC}        # for managed Deno binaries\n\n"
printf "zsh users (oh-my-zsh / similar): add ${CYAN}unset -f d${NC} after your PATH exports ('d' may be defined as a function).\n"
