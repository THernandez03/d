...
pub fn list_local() -> Result<()> {
    ...
    if versions.is_empty() {
        println!("No cached Deno versions found.");
        println!("Run `d install <version>` to install one.");
        return Ok(());
    }

    println!("Cached Deno versions:");
    for v in &versions {
...
pub fn interactive_picker() -> Result<()> {
    ...
    if versions.is_empty() {
        println!("No cached Deno versions found.");
        println!("Run `d install <version>` or `d ls-remote` to get started.");
        return Ok(());
    }
    ...
    let selection = Select::with_theme(&ColorfulTheme::default())
        .with_prompt("Select a Deno version (arrow keys, Enter to install, q to quit)")
        .default(default)
        .items(&items)
        .interact_opt()?
        .unwrap_or(usize::MAX);
...
mod tests {
    ...
    fn with_temp_env<F: FnOnce()>(cache_dir: &std::path::Path, prefix_dir: &std::path::Path, f: F) {
        ...
        std::env::set_var("D_CACHE_DIR", cache_dir);
        std::env::set_var("D_PREFIX", prefix_dir);
        f();
    ...
    fn format_versions_no_active() {
        let versions = ["v2.0.0".to_string(), "v1.40.0".to_string()];
        let lines = format_versions(&versions, None);
        assert_eq!(lines, vec!["  v2.0.0", "  v1.40.0"]);
    }

    #[test]
    fn format_versions_marks_active() {
        let versions = ["v2.0.0".to_string(), "v1.40.0".to_string()];
        let lines = format_versions(&versions, Some("v2.0.0"));
        assert_eq!(lines[0], "  v2.0.0 (active)");
        assert_eq!(lines[1], "  v1.40.0");
    }

    #[test]
    fn format_items_no_active() {
        let versions = ["v2.0.0".to_string()];
        let items = format_items(&versions, None);
        assert_eq!(items, vec!["v2.0.0"]);
    }

    #[test]
    fn format_items_marks_active_with_star() {
        let versions = ["v2.0.0".to_string(), "v1.40.0".to_string()];
        let items = format_items(&versions, Some("v2.0.0"));
        assert_eq!(items[0], "v2.0.0 *");
        assert_eq!(items[1], "v1.40.0");
    }
    ...
    fn list_local_works_with_versions() {
        let cache_dir = tempfile::tempdir().expect("tempdir");
        let prefix_dir = tempfile::tempdir().expect("tempdir");
        fs::create_dir_all(cache_dir.path().join("v1.40.0")).unwrap();
        with_temp_env(cache_dir.path(), prefix_dir.path(), || {
            assert!(super::list_local().is_ok());
        });
    }
