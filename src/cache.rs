...
pub fn which(tag: &str) -> Result<PathBuf> {
    ...
    if path.exists() {
        ...
        anyhow::bail!("Version '{tag}' is not cached. Run `d {tag}` to install it.")
    }
}
...
/// (partial version, e.g. `"2.3"` matches `"2.3.6"`).
...
pub fn find_by_sha(sha: &str) -> Option<String> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(cached_sha) = cache_key_sha(&name) {
            if sha_matches(cached_sha, sha) && deno_binary(&name).exists() {
                let mtime = entry
                    .metadata()
                    .ok()
                    .and_then(|m| m.modified().ok())
...
mod tests {
    ...
    fn prune_force_removes_all_versions() {
        ...
        fs::write(prefix.path().join(".active"), "v1.40.0").unwrap();
        fs::create_dir_all(cache.path().join("v1.40.0")).unwrap();
        fs::create_dir_all(cache.path().join("v1.38.0")).unwrap();

        prune(true).unwrap();

        assert!(
            !cache.path().join("v1.40.0").exists(),
            "--force should remove active"
        );
        assert!(
            !cache.path().join("v1.38.0").exists(),
    ...
    fn cache_key_sha_present() {
        assert_eq!(cache_key_sha("2.3.6+abc1234def"), Some("abc1234def"));
    }

    #[test]
    fn cache_key_sha_absent() {
        assert!(cache_key_sha("2.3.6").is_none());
    }
    ...
    fn find_by_version_prefix_exact() {
        with_temp_cache(|_| {
            make_cached_deno("2.3.6");
            assert_eq!(find_by_version_prefix("2.3.6"), Some("2.3.6".to_string()));
        });
    }
    ...
    fn find_by_version_prefix_with_sha_suffix() {
        with_temp_cache(|_| {
            make_cached_deno("2.3.6+abc1234def");
            assert_eq!(
                find_by_version_prefix("2.3.6"),
                Some("2.3.6+abc1234def".to_string())
            );
        });
    }
    ...
    fn find_by_version_prefix_dot_match() {
        with_temp_cache(|_| {
            make_cached_deno("2.3.6");
            assert_eq!(find_by_version_prefix("2.3"), Some("2.3.6".to_string()));
        });
    }
    ...
    fn find_by_version_prefix_no_match() {
        with_temp_cache(|_| {
            make_cached_deno("2.3.6");
            assert!(find_by_version_prefix("1.40.0").is_none());
        });
    }
    ...
    fn find_by_version_prefix_requires_binary() {
        with_temp_cache(|base| {
            fs::create_dir_all(base.join("2.3.6")).unwrap();
            assert!(find_by_version_prefix("2.3.6").is_none());
        });
    }
    ...
    fn find_by_sha_input_prefix_of_stored() {
        with_temp_cache(|_| {
            make_cached_deno("2.3.6+abc1234def5678");
            assert_eq!(
                find_by_sha("abc1234d"),
                Some("2.3.6+abc1234def5678".to_string())
            );
        });
    }
    ...
    fn find_by_sha_stored_prefix_of_input() {
        with_temp_cache(|_| {
            make_cached_deno("2.3.6+abc1234d");
            assert_eq!(
                find_by_sha("abc1234def5678"),
                Some("2.3.6+abc1234d".to_string())
            );
        });
    }
    ...
    fn find_by_sha_ignores_entry_without_sha() {
        with_temp_cache(|_| {
            make_cached_deno("2.3.6");
            assert!(find_by_sha("23600").is_none());
        });
    }
    ...
    fn rename_version_moves_dir() {
        with_temp_cache(|base| {
            fs::create_dir_all(base.join("2.3.6")).unwrap();
            rename_version("2.3.6", "2.3.6+abc1234def").unwrap();
            assert!(!base.join("2.3.6").exists());
            assert!(base.join("2.3.6+abc1234def").exists());
        });
    }
    ...
    fn rename_version_noop_when_new_exists() {
        with_temp_cache(|base| {
            fs::create_dir_all(base.join("2.3.6")).unwrap();
            fs::create_dir_all(base.join("2.3.6+abc1234def")).unwrap();
            rename_version("2.3.6", "2.3.6+abc1234def").unwrap();
            assert!(base.join("2.3.6").exists());
            assert!(base.join("2.3.6+abc1234def").exists());
        });
